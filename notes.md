# 🧠 notes

Collected lessons, study advice, and memorization techniques from others who passed the CISSP.
This page exists to reduce friction, not to be perfect. Use what sticks. Ignore the rest.

---

## 📌 Lessons Learned From Others (CISSP Pass Stories)

_Source:_  
https://www.reddit.com/r/cissp/comments/1bwha75/passed_cissp_1st_attempt_with_10_weeks_of_prep_my/

### 1. Prioritize Practice Problems
Do more questions. Early failure is part of learning.

- Don’t avoid practice exams because you’re afraid of getting questions wrong.
- Getting things wrong early is how gaps are exposed.
- Practice problems build confidence faster than passive reading.

---

### 2. Practice Your Pacing
Time management matters more than you think.
- Practice full-length exams (175 questions), not just 125.
- Use a fixed cadence: X seconds per question.
- Poor pacing can cost you the exam even if you know the material.

---

### 3. Destination Certification Materials
Highly effective for consolidation.

- Destination CISSP textbook and workbook are strong.
- The full crash course is worth it if budget allows.
- Especially useful for summarization and mental models.

---

### 4. Use Memorization Techniques Early
Understanding is important, but memory matters.

- CISSP has a *lot* of material.
- Mnemonics reduce cognitive load.
- Don’t wait until the last week to do this.

- [Pete Zerger - Memorization Techniques](https://www.youtube.com/watch?v=LGqZbiitiDw)

---

### 5. Review Notes Often
Your own words beat the textbook.

- Notes written in your own language stick better.
- Re-reading notes early prevents last-minute panic.

---

### 6. Practice the Math
Don’t fear formulas. Repetition fixes that.

- SLE, ALE, ARO, EF are easy once drilled.
- Practice eliminates hesitation.

📺 [Pete Zerger - CISSP Math](https://youtu.be/ttOKJYOedNo?si=uk71j4GALOgfxNPj)

---

### 7. Have a Study Buddy (Optional)
Not required, but helpful.

---

### 8. Avoid Doomscrolling r/CISSP
Too many stories cause noise.

---

### 🛑 Risk Management Framework (NIST 800-37)
**Please Can Superman Implode All Awful Millionaires**

- Prepare  
- Categorize  
- Select  
- Implement  
- Assess  
- Authorize  
- Monitor  

---

### 🧬 Risk Maturity Model
**Alien Pizza Doesn’t Ingest Organically**
- Ad Hoc  
- Preliminary  
- Defined  
- Integrated  
- Optimized  

---

### 🔄 Change Management
**RRATS!** (Richard Rarely Approves The Scheduled Deployment)

- Request  
- Review  
- Approve  
- Test  
- Schedule  
- Document  

---

### 📄 Information Lifecycle
Create → Classify → Store → Use → Archive → Destroy

---

### 🚨 Incident Response
**DRMRRRL**

- Detect  
- Respond  
- Mitigate  
- Report  
- Recover  
- Remediate  
- Lessons Learned  

---

### 🏢 Business Continuity Planning
**Save your BPA**

- Scope  
- BIA  
- Plan  
- Approve  

---

### 📈 Capability Maturity Model
Initialize → Repeatable → Defined → Managed → Optimized

---

### 🔧 IDEAL Model
Initiate → Diagnose → Establish → Act → Learn

---

### 🧑‍💻 SDLC
**Real Developers Ideas Take Effort**

- Requirements  
- Design  
- Implement  
- Test  
- Evolve  

---

### 🔥 Fire Classes
**All Boys Crave Doing Karate**

- Class A - Ordinary combustibles  
- Class B - Liquids  
- Class C - Electrical  
- Class D - Metals  
- Class K - Kitchen  

---

### 🌐 OSI Model
**Please Do Not Throw Sausage Pizza Away**

1. Physical  
2. Data Link  
3. Network  
4. Transport  
5. Session  
6. Presentation  
7. Application  

---

### 📦 OSI PDUs
**Definitely Some People Fear Bedbugs**

- Layers 5-7: Data  
- Layer 4: Segments  
- Layer 3: Packets  
- Layer 2: Frames  
- Layer 1: Bits  

---

### 🧱 Security Models Quick Hooks

- **Bell-LaPadula** - BL(UD) (Write Up, Read Down)  
- **Biba(DU)** - (No Write Up, No Read Down)  
- **Clark-Wilson** - Integrity + well-formed transactions  
- **Brewer-Nash** - Conflict of interest  

---

### 🧾 Common Criteria (EAL)
- EAL 1-2: Simple  
- EAL 3-4: Methodical  
- EAL 5-6: Semi-formal  
- EAL 7: Formal  

---

### 🧭 (ISC)² Code of Ethics
**PAPA**
- Protect society  
- Act honorably  
- Provide diligent service  
- Advance the profession  

---

## 🧠 Final Note
These mnemonics are not the exam.
They are **hooks** so your brain doesn’t freeze under pressure.

Understand first.  
Memorize just enough.  
Answer like a manager.

---
